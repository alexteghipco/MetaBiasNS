...
class MetaAnalysis(object):
    ...
    def __init__(self, dataset, ids, ids2=None, q=0.01, prior=0.5,
        ...
        if contra == 1:
            ...
            # extract significantly activated voxels
            v = np.nonzero(pFgA_z_FDR)[0]
            ce = np.zeros(pFgA_z_FDR.shape)
            # loop over voxels and see how many studies need to be added to break significance
            vii = 0
            for vi in v:
                vii = vii+1
                print(float(vii)/(len(v)))
                cl = cells[vi] # extract voxel-level contingency table
                c = 0 # start counter
                br = 0 # start condition for breaking while loop
                while br < 1: # init while loop
                    c = c+1 # counts number of simulated studies
                    cl[1,0] = cl[1,0]+1 # add a study showing contra evidence (ie study not in ids shows activity in this voxel)
                    p_sim = stats.two_way(cl[np.newaxis,...])
                    if p_sim > fdr_thresh:
                        br = 1
                ce[vi] = c
        else:
